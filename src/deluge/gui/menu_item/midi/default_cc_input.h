...
// "Default CC Input" - a per-instrument device+channel whose incoming CCs are interpreted via
// MIDI-follow's default CC->param map (see Instrument::defaultCCMidiInput / Drum::defaultCCMidiInput).
// The same menu item serves three contexts, resolved live:
//  - synth: the SoundInstrument's binding
//  - kit row (a specific drum selected): that SoundDrum's binding
//  - kit global FX: the Kit's binding
// Display + learn mirror the MIDI-follow channel menu. Channel-only (no MPE zone) by design.
class DefaultCCInput final : public Integer {
public:
	using Integer::Integer;

	LearnedMIDI& getLearnedMIDI() {
		if (soundEditor.editingKitRow()) {
			return static_cast<SoundDrum*>(soundEditor.currentSound)->defaultCCMidiInput;
		}
		return getCurrentInstrument()->defaultCCMidiInput;
	}

	// Highest selectable value. Plain MIDI channels are 0..15; values >= MIDI_CHANNEL_MPE_LOWER_ZONE (16)
	// are MPE zones, which this binding deliberately does not support (the default CC map is channel-based,
	// and LearnedMIDI would otherwise persist these as mpeZone="lower"/"upper").
...
private:
	// Captures the source device + channel (channel-only; the CC number is intentionally ignored, since
	// the default map covers every CC). Mirrors how midiInput is channel-learned.
